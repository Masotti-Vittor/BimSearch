...
public class AddElement extends javax.swing.JFrame {
    ...
    private void setupDragAndDrop() {
        pnlDrag.setTransferHandler(new TransferHandler() {
		    ...
		    selectedFile = imgFile;
                    ...
                    String newPath = copyToProjectFolder(imgFile);
                    currentImagePath = newPath;

                    // Atualizar a visualização (se quiser adicionar um JLabel no painel)
    ...
    return "br/com/bimsearch/windows/images/" + fileName;
}

	private void add(){
		
		String sql = "insert into images(image_path, numero_conexoes, argola, cabo, conduite, cabo_conduite, equipamento, aereo, tamanho) value (?,?,?,?,?,?,?,?,?)";

		try{
			pst = conec.prepareStatement(sql);
			
			if(selectedFile != null){
				pst.setString(1, copyToProjectFolder(selectedFile));
			}
			else{
				JOptionPane.showMessageDialog(null, "There is no path declared");
			}
			...
			// Create a column with the name of the element. (later).
